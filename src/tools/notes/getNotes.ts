/**
 * Get Notes Tool
 * Retrieve notes from JobNimbus /notes endpoint
 *
 ...
 */
function generateCacheIdentifier(input: GetNotesInput): string {
...
export class GetNotesTool extends BaseTool<GetNotesInput, any> {
  get definition(): MCPToolDefinition {
    return {
      name: 'get_notes',
      description: 'Retrieve notes from JobNimbus. Notes are text annotations attached to jobs, contacts, and other entities. Supports pagination, filtering, and sorting.',
      inputSchema: {
        type: 'object',
        properties: {
          from: {
  ...
  async execute(input: GetNotesInput, context: ToolContext): Promise<any> {
    const cacheIdentifier = generateCacheIdentifier(input);

    return await withCache(
      {
        entity: CACHE_PREFIXES.NOTES,
      ...
      async () => {
        ...
        const result = await this.client.get(context.apiKey, 'notes', params);
