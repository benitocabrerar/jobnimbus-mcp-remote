/**
 * Get Orders Tool
 * Retrieve orders from JobNimbus /orders endpoint
 *
 ...
 */
function generateCacheIdentifier(input: GetOrdersInput): string {
...
export class GetOrdersTool extends BaseTool<GetOrdersInput, any> {
  get definition(): MCPToolDefinition {
    return {
      name: 'get_orders',
      description: 'Retrieve orders from JobNimbus. Orders represent purchase orders and material orders associated with jobs. Supports pagination, filtering, and sorting.',
      inputSchema: {
        type: 'object',
        properties: {
          from: {
  ...
  async execute(input: GetOrdersInput, context: ToolContext): Promise<any> {
    const cacheIdentifier = generateCacheIdentifier(input);

    return await withCache(
      {
        entity: CACHE_PREFIXES.ORDERS,
      ...
      async () => {
        ...
        const result = await this.client.get(context.apiKey, 'orders', params);
